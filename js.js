...
document.addEventListener("mousemove", () => {
    if (!isCursorAnimating) { isCursorAnimating = true; animateCursor(); }
}, { once: false });

// Cursor grows on interactive elements
document.querySelectorAll("a, button, .color-btn").forEach(el => {
    el.addEventListener("mouseenter", () => { cursor.style.width="50px"; cursor.style.height="50px"; cursor.style.borderWidth="3px"; });
    el.addEventListener("mouseleave", () => { cursor.style.width="40px"; cursor.style.height="40px"; cursor.style.borderWidth="2px"; });
});

// Pulse on move
let lastMouseMoveTime = 0, pulseFrame = null;
function checkPulse() {
    if (Date.now() - lastMouseMoveTime > 100) { cursor.style.borderWidth = "2px"; pulseFrame = null; }
    else pulseFrame = requestAnimationFrame(checkPulse);
}
document.addEventListener("mousemove", () => {
...
function handleSubmit(e) {
    ...
    setTimeout(() => { btn.textContent = 'Send Message →'; btn.style.background=''; btn.style.borderColor=''; e.target.reset(); }, 3000);
}

// Animate skill bars on scroll
const skillFills = document.querySelectorAll('.skill-fill');
const skillObserver = new IntersectionObserver(entries => {
    entries.forEach(e => { if (e.isIntersecting) e.target.style.transform = 'scaleX(1)'; });
}, { threshold: 0.2 });
skillFills.forEach(el => skillObserver.observe(el));

// Animate section elements on scroll
const animEls = document.querySelectorAll('.section-label, .section-title, .about-card, .skill-category, .contact-left, .contact-right');
...
document.querySelectorAll('.color-btn[href^="#"]').forEach(link => {
...
const sections = [
    { id: 'about', link: document.querySelector('.color-btn[href="#about"]') },
    { id: 'skills', link: document.querySelector('.color-btn[href="#skills"]') },
    { id: 'contact', link: document.querySelector('.color-btn[href="#contact"]') },
];
...
// Scroll handler — fade hero text, show/hide home button
const heroEls = document.querySelectorAll('.heading, .scroll-hint');
...
window.addEventListener('scroll', () => {
    ...
    // Fade out hero name and scroll hint as user enters sections
    const progress = Math.min(Math.max((s - vh * 0.6) / (vh * 0.3), 0), 1);
    const opacity = 1 - progress;
    heroEls.forEach(el => {
        el.style.opacity = opacity;
        el.style.pointerEvents = opacity < 0.1 ? 'none' : '';
    });
    if (scrollHint) scrollHint.style.opacity = s > 40 ? '0' : '1';

    // Show home button once user scrolls into the sections
    const sectionsTop = sectionsWrapper ? sectionsWrapper.getBoundingClientRect().top : vh;
    if (sectionsTop < vh * 0.5) {
        navHome.classList.add('visible');
    } else {
        navHome.classList.remove('visible');
    }

    // Hide the canvas entirely once sections cover the screen — prevents
    // the gradient from bleeding through below the last section
    const canvas = document.getElementById('webGLApp');
    if (canvas) {
        const sectionsTop2 = sectionsWrapper ? sectionsWrapper.getBoundingClientRect().top : vh;
        if (sectionsTop2 <= 0) {
